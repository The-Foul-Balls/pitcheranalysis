...
median_by_closer <- data.frame(name=character(), pitch=character(), median_spin=numeric(), median_velo=numeric(),
                              ...
                              median_plheight=numeric(), pitch_count=numeric(), pitcherthrows = character(),  stringsAsFactors=FALSE)
...
for (i in closer_tables) {
  ...
  for (j in table_by_pitch) {
    ...
    pitcherthrows <- unique(j$pitcherthrows)[1]
    ...
    median_by_closer <- rbind(median_by_closer,
                             data.frame(name=i$pitcher[1], pitch=j$pitchname[1], median_spin=median_spin,
                                        ...
                                        median_plside=median_plside, median_plheight=median_plheight, pitch_count=pitch_count, pitcherthrows = pitcherthrows))
  }
}
...
med_sl_by_reliever <- filter(median_by_reliever, pitch == "SL", pitch_count >= 30) |>
  mutate(type = "rel") |>
  mutate(abs_med_hbreak= abs(median_hbreak))|>
  mutate(log_abs_med_hbreak= log(abs_med_hbreak))|>
  mutate(abs_med_vbreak= abs(median_vbreak))|>
  mutate(log_abs_med_vbreak= log(abs_med_vbreak))

med_sl_by_closer <- filter(median_by_closer, pitch == "SL") |>
  mutate(type = "close")|>
  mutate(abs_med_hbreak= abs(median_hbreak))|>
  mutate(log_abs_med_hbreak= log(abs_med_hbreak))|>
  mutate(abs_med_vbreak= abs(median_vbreak))|>
...
### Horizontal Break models and residuals [log(x+k) transformation]

```{r}
# # Create a new response variable as the log transform of 'median_hbreak'
# med_sl_by_pitcher <- med_sl_by_pitcher|>
#   mutate(adj_med_hbreak= median_hbreak+ 30) |> #makes everything positive to take log
#   mutate(log_adj_med_hbreak= log(adj_med_hbreak))
# 
# ## Basic Log Model
# hlog_model <- glm(log_adj_med_hbreak ~ median_spin + median_velo + median_vbreak, 
#                       family = gaussian(link = "identity"), 
#                        data = med_sl_by_pitcher)
# summary(hlog_model)
# 
# hresiduals_log_model <- residuals(hlog_model)
# hfitted_values_log_model <- fitted(hlog_model)
# 
# plot(hfitted_values_log_model, hresiduals_log_model,
#      xlab = "Fitted Values",
#      ylab = "H Break Residuals",
#      main = "Residuals vs Fitted Values",
#      pch = 16, col = "blue")
# abline(h = 0, col = "red", lwd = 2)
# 
# length(med_sl_by_pitcher$log_adj_med_hbreak) == length(residuals(hlog_model))
# med_sl_by_pitcher <- med_sl_by_pitcher |> 
#    mutate(hbreak_resid = residuals(hlog_model))
# 
# 
# ## Interaction Model 1
# log_model_int <- glm(log_adj_med_hbreak ~ median_spin + median_velo + median_vbreak + median_spin*median_velo, 
#                       family = gaussian(link = "identity"), 
#                        data = med_sl_by_pitcher)
# summary(log_model_int)
# 
# residuals_log_model_int <- residuals(log_model_int)
# fitted_values_log_model_int <- fitted(log_model_int)
# 
# plot(fitted_values_log_model_int, residuals_log_model_int,
#      xlab = "Fitted Values",
#      ylab = "Residuals",
#      main = "Residuals vs Fitted Values",
#      pch = 16, col = "blue")
...
# ## Interaction Model 2
# 
# log_model_int_2 <- glm(log_adj_med_hbreak ~ median_spin + median_velo + median_vbreak + median_spin*median_velo  + median_velo*median_vbreak, 
#                       family = gaussian(link = "identity"), 
#                        data = med_sl_by_pitcher)
# summary(log_model_int_2)
# 
# residuals_log_model_int_2 <- residuals(log_model_int_2)
# fitted_values_log_model_int_2 <- fitted(log_model_int_2)
# 
# plot(fitted_values_log_model_int_2, residuals_log_model_int_2,
#      xlab = "Fitted Values",
#      ylab = "Residuals",
#      main = "Residuals vs Fitted Values",
#      pch = 16, col = "blue")
...
### Vertical Break models & residuals [log(x+k) transformation]

```{r}
# # Create a new response variable as the log transform of 'median_vbreak'
# med_sl_by_pitcher <- med_sl_by_pitcher|>
#   mutate(adj_med_vbreak= median_vbreak+8) |> #makes everything positive to take log
#   mutate(log_adj_med_vbreak= log(adj_med_vbreak))
# 
# ## Basic Log Model
# vlog_model <- glm(log_adj_med_vbreak ~ median_spin + median_velo + median_hbreak, 
#                       family = gaussian(link = "identity"), 
#                        data = med_sl_by_pitcher)
# summary(vlog_model)
# 
# vresiduals_log_model <- residuals(vlog_model)
# vfitted_values_log_model <- fitted(vlog_model)
# 
# plot(vfitted_values_log_model, vresiduals_log_model,
#      xlab = "Fitted Values",
#      ylab = "V Break Residuals",
#      main = "Residuals vs Fitted Values",
#      pch = 16, col = "blue")
# abline(h = 0, col = "red", lwd = 2)
# 
# length(med_sl_by_pitcher$log_adj_med_vbreak) == length(residuals(vlog_model))
# med_sl_by_pitcher <- med_sl_by_pitcher |> 
#    mutate(vbreak_resid = residuals(vlog_model))
# 
# ## Interaction Model 1
# log_model_int <- glm(log_adj_med_vbreak ~ median_spin + median_velo + median_hbreak + median_spin*median_velo, 
#                       family = gaussian(link = "identity"), 
#                        data = med_sl_by_pitcher)
# summary(log_model_int)
# 
...
# ## Interaction Model 2
# 
# log_model_int_2 <- glm(log_adj_med_vbreak ~ median_spin + median_velo + median_hbreak + median_spin*median_velo  + median_velo*median_hbreak, 
#                       family = gaussian(link = "identity"), 
#                        data = med_sl_by_pitcher)
# summary(log_model_int_2)
# 
...
#   select(median_spin, median_velo, median_hbreak, median_vbreak, vbreak_resid, hbreak_resid) %>%
#   scale() |>
#   as.data.frame()
# 
...
#   labs(title = "Slider Pitching Stats Histograms", y = "Frequency", x = "Standardized Values") 

```

### log(abs()) transformation
```{r}
# med_sl_by_closer$vbreak_sign <- factor(ifelse(med_sl_by_closer$median_vbreak > 0, "pos", "neg"))
# med_sl_by_closer$hbreak_sign <- factor(ifelse(med_sl_by_closer$median_hbreak > 0, "pos", "neg"))
# med_sl_by_reliever$vbreak_sign <- factor(ifelse(med_sl_by_reliever$median_vbreak > 0, "pos", "neg"))
# med_sl_by_reliever$hbreak_sign <- factor(ifelse(med_sl_by_reliever$median_hbreak > 0, "pos", "neg"))
...
v_logabs_model <- glm(log_abs_med_vbreak ~ median_spin + median_velo+ median_hbreak, 
                      family = gaussian(link = "identity"), 
                       data = med_sl_by_pitcher)
summary(v_logabs_model)

vresiduals_logabs <- residuals(v_logabs_model)
vfitted_values_logabs <- fitted(v_logabs_model)

plot(vfitted_values_logabs, vresiduals_logabs,
     xlab = "Fitted Values",
     ylab = "V Break Residuals",
     main = "Residuals vs Fitted Values",
     pch = 16, col = "blue")
...
med_sl_by_pitcher <- med_sl_by_pitcher |> 
   mutate(logabs_vbreak_resid = residuals(v_logabs_model))

#v interaction term 1

v_logabs_model1 <- glm(log_abs_med_vbreak ~ median_spin + median_velo+ median_hbreak + median_spin*median_velo, 
                      family = gaussian(link = "identity"), 
                       data = med_sl_by_pitcher)
summary(v_logabs_model1)

vresiduals_logabs1 <- residuals(v_logabs_model1)
vfitted_values_logabs1 <- fitted(v_logabs_model1)

plot(vfitted_values_logabs1, vresiduals_logabs1,
     xlab = "Fitted Values",
     ylab = "V Break Residuals",
     main = "Residuals vs Fitted Values",
     pch = 16, col = "blue")
...
med_sl_by_pitcher <- med_sl_by_pitcher |> 
   mutate(logabs_vbreak_resid_1 = residuals(v_logabs_model1))

#v interaction term 2

v_logabs_model2 <- glm(log_abs_med_vbreak ~ median_spin + median_velo+ median_hbreak + median_spin*median_velo  + median_velo*median_hbreak, 
                      family = gaussian(link = "identity"), 
                       data = med_sl_by_pitcher)
summary(v_logabs_model2)

vresiduals_logabs2 <- residuals(v_logabs_model2)
vfitted_values_logabs2 <- fitted(v_logabs_model2)

plot(vfitted_values_logabs2, vresiduals_logabs2,
     xlab = "Fitted Values",
     ylab = "V Break Residuals",
     main = "Residuals vs Fitted Values",
     pch = 16, col = "blue")
...
med_sl_by_pitcher <- med_sl_by_pitcher |> 
   mutate(logabs_vbreak_resid_2 = residuals(v_logabs_model2))

summary.lm(v_logabs_model)
summary.lm(v_logabs_model1)
summary.lm(v_logabs_model2) #this is the one
...
h_logabs_model <- glm(log_abs_med_hbreak ~ median_spin + median_velo + median_vbreak, 
                      family = gaussian(link = "identity"), 
                       data = med_sl_by_pitcher)
summary(h_logabs_model)

hresiduals_logabs <- residuals(h_logabs_model)
hfitted_values_logabs <- fitted(h_logabs_model)

plot(hfitted_values_logabs, hresiduals_logabs,
     xlab = "Fitted Values",
     ylab = "H Break Residuals",
     main = "Residuals vs Fitted Values",
     pch = 16, col = "blue")
...
med_sl_by_pitcher <- med_sl_by_pitcher |> 
   mutate(logabs_hbreak_resid = residuals(h_logabs_model))

#h interaction term 1

h_logabs_model1 <- glm(log_abs_med_hbreak ~ median_spin + median_velo+ median_vbreak + median_spin*median_velo, 
                      family = gaussian(link = "identity"), 
                       data = med_sl_by_pitcher)
summary(h_logabs_model1)

hresiduals_logabs1 <- residuals(h_logabs_model1)
hfitted_values_logabs1 <- fitted(h_logabs_model1)

plot(hfitted_values_logabs1, hresiduals_logabs1,
     xlab = "Fitted Values",
     ylab = "H Break Residuals",
     main = "Residuals vs Fitted Values",
     pch = 16, col = "blue")
...
med_sl_by_pitcher <- med_sl_by_pitcher |> 
   mutate(logabs_hbreak_resid_1 = residuals(h_logabs_model1))

#h interaction term 2

h_logabs_model2 <- glm(log_abs_med_hbreak ~ median_spin + median_velo+ median_vbreak + median_spin*median_velo  + median_velo*median_vbreak, 
                      family = gaussian(link = "identity"), 
                       data = med_sl_by_pitcher)
summary(h_logabs_model2)

hresiduals_logabs2 <- residuals(h_logabs_model2)
hfitted_values_logabs2 <- fitted(h_logabs_model2)

plot(hfitted_values_logabs2, hresiduals_logabs2,
     xlab = "Fitted Values",
     ylab = "H Break Residuals",
     main = "Residuals vs Fitted Values",
     pch = 16, col = "blue")
...
med_sl_by_pitcher <- med_sl_by_pitcher |> 
   mutate(logabs_hbreak_resid_2 = residuals(h_logabs_model2))

summary.lm(h_logabs_model)
summary.lm(h_logabs_model1) #this is the one
summary.lm(h_logabs_model2)
```
...
#histograms
med_sl_by_pitcher_scaled <- med_sl_by_pitcher |>
  select(median_spin, median_velo, median_hbreak, median_vbreak, logabs_hbreak_resid_1, logabs_vbreak_resid_2) |> 
  scale() |>
  as.data.frame()
