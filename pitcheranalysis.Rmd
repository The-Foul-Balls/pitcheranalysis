...
rv100 <- read.csv("data/rv100data.csv")

result <- med_sl_by_pitcher %>%
  inner_join(rv100[,c(1,3)], by = "name")

sameNamediffPlayer <- result %>%
  group_by(name, type) %>%
  filter(n() > 1) %>%
  ungroup()
...
med_sl_by_pitcher <- med_sl_by_pitcher |> 
   mutate(logabs_vbreak_resid = residuals(log_model_int_2))
